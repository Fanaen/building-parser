...
use crate::express::primitives::{PrimitiveType, Aggregation, Enumeration, Select};
...
impl DefinedDataType {
    pub fn from_pair(pair: Pair<Rule>) -> DefinedDataType {
        ...
        let data_type = DefinableDataType::from_pair(tokens.next().expect("DefinedDataType should have an underlying data type"));
...
impl DefinableDataType {
    pub fn from_pair(pair: Pair<Rule>) -> DefinableDataType {
        match pair.as_rule() {
            Rule::primitive_type => DefinableDataType::PrimitiveType(PrimitiveType::from_pair(pair)),
